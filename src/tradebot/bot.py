...
class FyersBroker(Broker):
    def __init__(self, settings: Settings) -> None:
        ...
        log.info(
            "Fyers init: client_id=%s token_len=%s token_prefix=%s",
            self.client_id,
            len(self.access_token),
            self.access_token[:3] + "***" if self.access_token else "EMPTY",
        )
    ...
    def _raise_fyers_error(self, where: str, response: Dict[str, Any]) -> None:
        ...
        if code == -15:
            raise RuntimeError(
                f"Fyers auth failed in {where}: invalid/expired access token. "
                "Regenerate access_token from Fyers auth flow and update FYERS_ACCESS_TOKEN."
            )
...
def good_morning_message() -> str:
    return (
        ...
        "🔔 Channel active rakho. Best move fast aata hai."
        + brand_signature()
    )
...
def ready_alert_message() -> str:
    return (
        ...
        "🔥 Late entry avoid\n\n"
        "Phone ready rakho. Jiga Bhai desk active hai."
        + brand_signature()
    )
...
def opening_plan_message(level_rows: List[Dict[str, Any]], vip_link: str) -> str:
    ...
    for row in level_rows:
        rows_text.append(
            ...
            f"Buy Above 🔸 <b>{fmt(row['breakout'])}</b>\n"
            f"Weak Below 🔸 <b>{fmt(row['breakdown'])}</b>\n"
            f"Key Zone 🔸 <b>{fmt(row['support'])}</b> - <b>{fmt(row['resistance'])}</b>"
        )
    levels = "\n\n".join(rows_text) if rows_text else "Live levels price action ke saath update honge."
    return (
        "🚨 <b>FIRST MARKET PLAN</b> 🚨\n"
        "⏰ <b>9:15 - 9:30 WINDOW</b>\n\n"
        f"{levels}\n\n"
        "🎯 Clean breakout mila to entry.\n"
        "🛡 Confirmation nahi to no trade.\n"
        "🔥 Capital safe, profit aggressive.\n\n"
        f"👑 VIP Desk: {escape(vip_link)}"
        + brand_signature()
    )
...
def next_day_plan_message(level_rows: List[Dict[str, Any]], vip_link: str) -> str:
    ...
    for row in level_rows:
        rows_text.append(
            f"🚩 <b>{escape(row['name'])}</b>\n"
            f"Resistance 🔸 <b>{fmt(row['resistance'])}</b>\n"
            f"Support 🔸 <b>{fmt(row['support'])}</b>\n"
            f"Breakout 🔸 <b>{fmt(row['breakout'])}+</b> | Breakdown 🔸 <b>{fmt(row['breakdown'])}-</b>"
        )

    levels = "\n\n".join(rows_text) if rows_text else "Fresh levels live market me update honge."

    return (
        f"🌙 <b>NEXT DAY LEVELS - {escape(next_trading_day_label())}</b> 🌙\n\n"
        f"{levels}\n\n"
        "🎯 First 15 min wait. Fake move me entry nahi.\n"
        "🚀 Breakout + volume = action zone.\n"
        "🛡 SL strict. Overtrade zero.\n\n"
        f"👑 VIP Priority Levels: {escape(vip_link)}"
        + brand_signature()
    )
    ...
    async def calculate_level_rows(self, limit: int = 2) -> List[Dict[str, Any]]:
        ...
        for idx, symbol in enumerate(symbols[:limit]):
            ...
            try:
                ...
                log.warning("Level calculation failed for %s: %s", symbol, exc)
        return level_rows

    async def opening_plan_job(self, context: ContextTypes.DEFAULT_TYPE) -> None:
        if not is_weekday():
            return
        level_rows = await self.calculate_level_rows(limit=2)
        await context.bot.send_message(
            chat_id=self.settings.telegram_chat_id,
            text=opening_plan_message(level_rows, self.settings.vip_link),
...
async def post_init(app: Application) -> None:
    ...
    app.job_queue.run_daily(
        manager.opening_plan_job,
        time=time(9, 16, tzinfo=IST),
        days=(0, 1, 2, 3, 4),
        name="first-sniper-plan",
    )
    app.job_queue.run_daily(
    ...
    log.info(
        "Jobs scheduled: daily 08:00/08:10/09:00/09:16/22:00 IST, scan=%ss trail=%ss",
