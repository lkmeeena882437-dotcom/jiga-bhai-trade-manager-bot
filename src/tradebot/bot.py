...
def should_cover_match(match):
    """v25.6: Cover EVERY real match (priority >= 3). Only skip youth/academy.
...
async def _fetch_from_highlightly():
    """v21: Highlightly API — 100 free req/day, 900+ leagues incl. IPL, T20 Blast, ECS."""
    if not HIGHLIGHTLY_API_KEY: return None
    # v25.6: Fetch 3 days (yesterday-ish coverage + today + tomorrow)
    today = now_ist().date()
    ...
    try:
        async with aiohttp.ClientSession() as s:
            for d in dates:
                url = f"https://{HIGHLIGHTLY_HOST}/matches?date={d}&limit=100"
                try:
...
async def job_discover_matches(bot, scheduler):
    ...
    if tk not in SCHEDULED_JOBS and matches:
        ...
        max_pick = min(8, len(sorted_matches))
        min_pick = min(3, max_pick)
        pick_count = random.choices(
            list(range(min_pick, max_pick + 1)),
            weights=[max(1, 5 - abs(x - 6)) for x in range(min_pick, max_pick + 1)]
        )[0]
    ...
    now = now_ist()
    immediate_count = 0  # v25.1: cap immediate-post backlog per discovery run
    for m in matches:
...
def main():
    ...
    logger.info("=" * 50)
    logger.info(f"🚀 {BOT_NAME} v25.6 MAX-COVERAGE starting...")
    logger.info("=" * 50)
    ai_count = 0
